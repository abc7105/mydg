...
var
  S1, S2, S2t, S2b : PRGBAArray;
  t, b : boolean;
  X, Y, sw, sh: Integer;
  function BlackPoint(c: TsColor_) : boolean;
  begin
    ...
    if (HintKind.GradientPercent > 0) then begin
      if iDrawed then begin
        ...
        try
          ...
          SumBmpRect(aBmp, Bmp, IntToByte(HintKind.TexturePercent shl 8 div 100), Rect(0, 0, Bmp.Width - 1, Bmp.Height{ - 1}), Point(aRect.Left, aRect.Top));
        finally
          FreeAndNil(Bmp);
        end;
      end
...
begin
  ...
  if ci.Ready and (Transparency = 100) then begin
    ...
    try
      ...
      SumBmpRect(BGBmp, TempBmp, IntToByte(Transparency shl 8 div 100), Rect(0, 0, WidthOf(aRect), HeightOf(aRect)), Point(aRect.Left, aRect.Top));
...
begin
  if FTempHint <> nil then FreeAndNil(FTempHint);
end;
...
{$ENDIF}
  ...
  var
    x, y : integer;
    FastDst : TacFast32;
    FastShadow : TacFast32;
    FastMask : TacFast32;
    FastBody : TacFast32;
    c : TsColor;
  begin
    FBlend.SourceConstantAlpha := Round(MaxByte - 2.55 * Manager.HintKind.Transparency);
    FreeAndNil(AlphaBmp);
    ...
    FastShadow := TacFast32.Create;
    FastMask := TacFast32.Create;
    FastBody := TacFast32.Create;
    ...
    if FastDst.Attach(AlphaBmp) and FastShadow.Attach(Manager.FCacheBmp) and FastMask.Attach(MaskBmp) and FastBody.Attach(BodyBmp) then begin
      for y := 0 to h - 1 do for x := 0 to w - 1 do begin
        if FastMask.Pixels[x, y].R = MaxByte then begin
          c.I := 0;
          c.A := MaxByte - FastShadow.Pixels[x, y].R;
        end
        else begin
...
{$IFDEF DELPHI6UP}
  ...
  if Rect.Bottom > Monitor.Top + Monitor.Height then begin
    if (Manager.HintKind.Style in [hsComics, hsEllipse, hsBalloon]) then begin
      Rect.Top := p.Y - h;
    end
    else Rect.Top := Monitor.Top + Monitor.Height - h;
    t := False;
    Auto := True
  end;
  ...
  if Rect.Right > Monitor.Left + Monitor.Width then begin
    l := False;
    Auto := True;
    if (Manager.HintKind.Style in [hsComics, hsEllipse, hsBalloon]) and (FMousePos = mpLeftTop) then begin
      FMousePos := mpRightTop;
      Rect.Left := p.X - w;
      Rect.Right := Rect.Left + w;
    end
    else Rect.Left := Monitor.Left + Monitor.Width - w;
  end;
...
{$ENDIF}
  ...
  if acLayered then begin
    ...
    if Manager.Animated and IsNTFamily and not Shown then begin
{$IFNDEF ACHINTS}
...
{$ENDIF}
    ...
    if not Assigned(ScreenBmp) then ScreenBmp := CreateBmp32(w, h) else begin ScreenBmp.Width := w; ScreenBmp.Height := h end;
...
begin
  ...
  if HintFrame <> nil then begin
    ...
    TempBmp := CreateBmp32(HintFrame.Width, HintFrame.Height);

    if (DefaultManager <> nil) and DefaultManager.Active then begin // If frame is skinned and WM_PRINT is supported
      ...
      TempBmp.Canvas.Lock;
      for i := 0 to HintFrame.ControlCount - 1 do begin
        if HintFrame.Controls[i] is TWinControl then begin
          TWinControl(HintFrame.Controls[i]).PaintTo(TempBmp.Canvas.Handle, HintFrame.Controls[i].Left, HintFrame.Controls[i].Top);
        end
        else begin
          MoveWindowOrg(TempBmp.Canvas.Handle, HintFrame.Controls[i].Left, HintFrame.Controls[i].Top);
          HintFrame.Controls[i].Perform(WM_PAINT, Longint(TempBmp.Canvas.Handle), 0);
          MoveWindowOrg(TempBmp.Canvas.Handle, -HintFrame.Controls[i].Left, -HintFrame.Controls[i].Top);
        end;
      end;
      TempBmp.Canvas.UnLock;
...
{$IFNDEF ACHINTS}
  ...
  if Fast32Src.Attach(Result) then begin
    h := Result.Height - 1;
    w := Result.Width - 1;
    for y := 0 to h do
      for x := 0 to w do
        if Fast32Src.Pixels[x, y].I = CI.FillColor
          then Fast32Src.Pixels[x, y] := White
          else Fast32Src.Pixels[x, y] := Black;
  end;
  Fast32Src.Free;
{$ENDIF}
end;
