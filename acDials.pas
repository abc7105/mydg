    ...
    HCBT_CREATEWND : begin
      ...
      case l of
        32768 : {$IFNDEF NOMNUHOOK} begin // Menu
          if {not AeroIsEnabled and} DefaultManager.SkinnedPopups then begin
...
begin
  ...
  if FindFormInList(hwnd) = nil then begin
    ...
    if (Form <> nil) and not (csLoading in Form.ComponentState) {and not (csRecreating in Form.ControlState) }then begin
      ...
      if not (srStdDialogs in DefaultManager.SkinningRules) then Exit;
      if (VisibleDlgCount > 1) and not ControlExists(hwnd, 'toolbarwindow32') then begin
        Result := False;
        Exit;
      end;
...
{        else if TWinControl(Wnd.SkinData.FOwnerControl).Controls[i] is TSpeedButton then begin
          TSpeedButton(TWinControl(Wnd.SkinData.FOwnerControl).Controls[i]).Flat := True;
          TSpeedButton(TWinControl(Wnd.SkinData.FOwnerControl).Controls[i]).Transparent := True;
        end}
...
{  else if (s = 'msctls_trackbar32') then begin
    Result := False; // Is not supported
  end}
  else if pClassName = 'systabcontrol32' then begin
    if ((Style and TCS_OWNERDRAWFIXED) <> TCS_OWNERDRAWFIXED) then begin
//      Wnd := TacPageControlWnd.Create(aHwnd, nil, ListSW.SkinData.SkinManager, s_PageControl);
//      TacPageControlWnd(Wnd).Provider := Self;
...
begin
  ...
  while hCtrl <> 0 do begin
    ...
    if ((GetWindowLong(hCtrl, GWL_STYLE) and WS_CHILD) = WS_CHILD) and ((GetWindowLong(hCtrl, GWL_STYLE) and WS_VISIBLE) = WS_VISIBLE) then begin
      SendAMessage(hCtrl, AC_PRINTING, LongInt(DC));
      SendMessage(hCtrl, WM_PRINT, LongInt(DC), 0);
...
begin
  ...
  if (GetParent(Child) = eData.CtrlHandle) then begin
    ...
    if (Style and WS_VISIBLE = WS_VISIBLE) then begin
      ...
      if (Style and BS_GROUPBOX = BS_GROUPBOX) then Exit
      else ExcludeClipRect(eData.DC, R.Left, R.Top, R.Right, R.Bottom);
    end;
  end;
end;
  ...
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    WM_NCLBUTTONDOWN : begin
      ...
      if (BorderForm <> nil) and (TWMNCLButtonDown(Message).HitTest = HTTRANSPARENT)
        ...
        else if (TWMNCLButtonDown(Message).HitTest <> HTBOTTOMRIGHT) { If statusbar with grip exists } then TWMNCLButtonDown(Message).HitTest := HTProcess(TWMNCHitTest(Message));
    ...
    WM_CTLCOLORDLG : begin
      Message.Result := LongInt(CreateSolidBrush(DefaultManager.GetGlobalColor));
      Exit;
    end;
    WM_DRAWITEM : begin
    ...
    WM_COMMAND : begin
      if (Message.WParam = 2) or (Message.WParam = WM_ACTIVATE) then begin
        if (SkinData.SkinManager.AnimEffects.DialogHide.Active) and (SkinData.SkinManager.AnimEffects.DialogHide.Time > 0) then begin
          PrindDlgClient(Self, SkinData.FCacheBmp, True);
          if BorderForm <> nil then begin
...
var
  i, TitleIndex, iDrawMode : integer;
  x, y, fHeight, fWidth, fCaptHeight, ChangedIndex : integer;
  s : acString;
  r, rForm : TRect;
  ci : TCacheInfo;
  ...
  begin
    if (CaptionHeight <> 0) then begin // Paint title
      if not exBorders then begin
        ...
        if SkinData.SkinManager.IsValidSkinIndex(TitleIndex) then begin
          if Iconic
            then PaintItem(TitleIndex, s_FormTitle, ci, True, integer(FormActive), Rect(rForm.Left, rForm.Top, rForm.Right, rForm.Bottom), Point(0, 0), SkinData.FCacheBmp, SkinData.SkinManager)
            else PaintItem(TitleIndex, s_FormTitle, ci, True, integer(FormActive), Rect(rForm.Left, rForm.Top, rForm.Right, fCaptHeight), Point(0, 0), SkinData.FCacheBmp, SkinData.SkinManager);
        end;
      end;
    ...
    if (CaptionHeight <> 0) then begin // Out the title text
      ...
      if Application.MainForm <> nil then SkinData.FCacheBmp.Canvas.Font.Charset := Application.MainForm.Font.Charset;
      ...
      if not IsRectEmpty(R) then begin
        ...
        else Ndx := SkinData.SkinManager.GetSkinIndex(s_FormTitle);
      ...
      if BorderForm <> nil then begin // Paint shadow of form if required
        ...
        if BorderForm.ShadowTemplate <> nil then begin
          if exBorders then with SkinData.SkinManager do begin
            ...
            if SkinData.BorderIndex > -1 then begin
              // Draw shadows in corners
              if SkinData.SkinManager.IsValidImgIndex(TitleIndex) then TitleIndex := SkinData.SkinManager.gd[TitleIndex].BorderIndex;
              if SkinData.SkinManager.IsValidImgIndex(TitleIndex) then begin // If title mask exists
                x := SkinData.SkinManager.MaskWidthRight(TitleIndex);
                // LeftTop
                R := Rect(ShadowSize.Left, ShadowSize.Top, ShadowSize.Left + SkinData.SkinManager.MaskWidthLeft(TitleIndex),
                          ShadowSize.Top + SkinData.SkinManager.MaskWidthTop(TitleIndex));
                FillTransPixels32(SkinData.FCacheBmp, BorderForm.ShadowTemplate, R, ShadowSize.TopLeft, TitleIndex, SkinData.SkinManager, HTTOPLEFT);
                // RightTop
                ...
                FillTransPixels32(SkinData.FCacheBmp, BorderForm.ShadowTemplate, R,
                  Point(max(0, BorderForm.ShadowTemplate.Width - ShadowSize.Right - x), ShadowSize.Top), TitleIndex, SkinData.SkinManager, HTTOPRIGHT);
              end
              else begin
...
begin                   
{$IFNDEF NOMNUHOOK}
  if MnuArray <> nil then for i := 0 to Length(MnuArray) - 1 do if (MnuArray[i] <> nil) then begin
