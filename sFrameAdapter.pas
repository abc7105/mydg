...
begin
  inherited;
  if Assigned(Frame) and (GetBoolMsg(Frame, AC_CTRLHANDLED) or not Frame.HandleAllocated) then SkinData.UpdateIndexes;
end;
...
{    AC_GETBG : if (FCommonData <> nil) then begin
  ...
  if (csDestroying in ComponentState) or (csDestroying in Frame.ComponentState) or not FCommonData.Skinned or not ((SkinData.SkinManager <> nil) and SkinData.SkinManager.SkinData.Active) then begin
    if SkinData.SkinManager <> nil then if SkinData.SkinManager.SkinData.Active then case Message.Msg of
      CM_SHOWINGCHANGED, CM_VISIBLECHANGED : begin
        if Assigned(Frame) and Frame.HandleAllocated then begin
          SkinData.UpdateIndexes;
          m.Msg := SM_ALPHACMD;
          m.WParam := MakeWParam(AC_SETNEWSKIN, AC_SETNEWSKIN);
          m.LParam := LongInt(SkinData.SkinManager);
          m.Result := 0;
    ...
    if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
      ...
      WM_SIZE, WM_MOVE : begin
        ...
        if Message.Msg = WM_SIZE then InvalidateRect(Frame.Handle, nil, True);
        OldWndProc(Message);
      end;
      WM_PARENTNOTIFY : if ((Message.WParam and $FFFF) in [WM_CREATE, WM_DESTROY]) and not (csLoading in ComponentState) and not (csCreating in Frame.ControlState) then begin
        if (Message.WParamLo = WM_CREATE) and Assigned(Frame) and Frame.HandleAllocated then begin
          SkinData.UpdateIndexes;
          m.Msg := SM_ALPHACMD;
...
begin
  if (aDC <> 0) and (not InAnimationProcess or (aDC = SkinData.PrintDC)) then begin
    ...
    if IsCached(FCommonData) then begin
      ...
      if aDC <> SkinData.PrintDC then DC := GetDC(Frame.Handle);
      try
        SavedDC := SaveDC(DC);
        ExcludeControls(DC, Frame, actGraphic, 0, 0);
        if i = 0 then FillBG(R) { Just fill BG} else begin
          if FCommonData.FCacheBmp = nil then FCommonData.FCacheBmp := CreateBmp32(Frame.Width, Frame.Height);
        ...
        if aDC <> SkinData.PrintDC then ReleaseDC(Frame.Handle, DC);
      end;
    end;
  end;
end;
