...
//    inc(ParentBG.Offset.x, SkinData.FOwnerControl.Left);
  ...
  if SkinData.BorderIndex > -1 then begin // if borders exists
    ...
    if SkinData.SkinManager.ma[SkinData.BorderIndex].MaskType = 1 then begin
      if ParentBG.BgType = btCache then begin
        BitBlt{Border}(SkinData.FCacheBmp.Canvas.Handle, aRect.Left, aRect.Top, WidthOf(aRect), HeightOf(aRect),
      ...
      else FillDCBorder(SkinData.FCacheBmp.Canvas.Handle, aRect, sm.ma[SkinData.BorderIndex].WL, sm.ma[SkinData.BorderIndex].WT, sm.ma[SkinData.BorderIndex].WR, sm.ma[SkinData.BorderIndex].WB, Color);
    end;
    DrawSkinRect(SkinData.FCacheBmp, aRect, False, BGInfoToCI(@ParentBG), SkinData.SkinManager.ma[SkinData.BorderIndex], State, True);
    Result := Rect(aRect.Left + sm.ma[SkinData.BorderIndex].WL, aRect.Top + sm.ma[SkinData.BorderIndex].WT, aRect.Right - sm.ma[SkinData.BorderIndex].WR, aRect.Bottom - sm.ma[SkinData.BorderIndex].WB);
    if MinBorderWidth <> 0 then begin
      ...
      FillDCBorder32(SkinData.FCacheBmp, Result, l, t, r, b, Color); // Fill skipped parts
    ...
    if SkinData.FCacheBmp.Canvas.Handle <> DC then begin
      if MinBorderWidth <> 0 then BitBltBorder(DC, aRect.Left, aRect.Top, WidthOf(aRect), HeightOf(aRect), SkinData.FCacheBmp.Canvas.Handle, aRect.Left, aRect.Top, MinBorderWidth);
    end;
  end
...
{          c.R := src.R;
          c.G := src.G;
          c.B := src.B;}
          if src.C = clFuchsia then begin // If transparent pixel..
...
{          c.R := src.R;
          c.G := src.G;
          c.B := src.B;}
          if src.C = sFuchsia.C then begin // If transparent pixel..
            if (Region <> 0) then begin
              if RegRect.Left <> -1 then inc(RegRect.Right) else begin
                RegRect.Left := DstX1 + X;
                RegRect.Right := RegRect.Left + 1;
...
begin
  ...
  // Fill
  if Filling and (MaskData.DrawMode and BDM_FILL = BDM_FILL) and (h > 0) and (w > 0) then begin
    if (MaskData.DrawMode and BDM_STRETCH = 0) or (MaskData.MaskType = 1) then begin
      y := wt;
      while y < Bmp.Height - h - wb do begin
        x := wl;
    ...
    else StretchBlt(Bmp.Canvas.Handle, wl, wt, Bmp.Width - wr - wl, Bmp.Height - wb - wt, BmpSrc.Canvas.Handle, dw + wl, MaskData.R.Top + wt, w, h, SRCCOPY);
  end;
end;
