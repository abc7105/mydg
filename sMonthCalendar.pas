...
{$IFNDEF NOTFORHELP}
  private
    ...
    FShowSelectAlways: boolean;
    FShowYearBtns: boolean;
    FShowMonthBtns: boolean;
    FMinDate: TDateTime;
    FMaxDate: TDateTime;
...
begin
  ...
  if s_Today = ''
    ...
    else FTodayBtn.Caption := s_Today + FormatDateTime(' mmmm d, yyyy', Date);
...
begin
  if ARow = 0 then begin
    if ShowWeeks and (ACol = 0) then Result := s_WeeksTitle else Result := ShortDayNames[(FirstDay + ACol - Integer(ShowWeeks) + 1) mod 7 + 1]
  end
...
var
  ...
  begin
    for i := aYear to aYear + 9 do begin
      ...
      if Items = PopMenu.Items then mi.Caption := '  ' + mi.Caption;
...
begin
  if FMaxDate <> Value then begin
    FMaxDate := Value;
    if Value <> 0 then begin
      if not (csLoading in ComponentState) and (CalendarDate > Value) then CalendarDate := Value;
    end;
    UpdateCalendar;
  end;
end;
...
begin
  Result := True;
  if (MinDate <> 0) and (Date < MinDate) then Result := False;
  if (MaxDate <> 0) and (Date > MaxDate) then Result := False;
end;
...
begin
  ...
  R := ARect;
  if ACol = 6 + Integer(FOwner.ShowWeeks) then R.Right := Width;
  ...
  if not FOwner.ShowSelectAlways and (gdSelected in AState) and not Focused then AState := AState - [gdSelected];
  ...
  if (ARow > 0) and (TheText <> '') and not (FOwner.ShowWeeks and (ACol = 0)) then begin
    ...
    if not FOwner.IsValidDate(d) then begin
