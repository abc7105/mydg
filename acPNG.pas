...
Uses Windows, Graphics, SysUtils, Classes, acZLibEx, Math, sConst;
...
function MakeCompIcon(Img: TPNGGraphic; BGColor : TColor = clNone): HICON;
...
begin
  ...
  Poly := 0;
  for N := 0 to SizeOf(P) - 1 do Poly := Poly or (1 shl (31 - P[N]));
...
begin
  ...
  if FAutoReset then FZLibResult := acZLibEX.InflateReset(FStream);
...
begin
  if UseACPng then with Stream do begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if Pass = 6 then
          begin
// 7.22            Progress(Self, psRunning, MulDiv(Row, 100, Height), True, FProgressRect, '');
...
begin
  ...
  FillDC(ColorBitmap.Canvas.Handle, Rect(0, 0, ColorBitmap.Width, ColorBitmap.Height), BGColor);

  Img.Draw(ColorBitmap.Canvas, Rect(0, 0, ColorBitmap.Width, ColorBitmap.Height));

  Fast32Src := TacFast32.Create;
  Fast32Dst := TacFast32.Create;
  if Img.PixelFormat <> pf32bit then begin
    Img.PixelFormat := pf32bit;
    if Fast32Src.Attach(Img) then begin // Make Alpha mask
      for Y := 0 to Img.Height - 1 do begin
        for X := 0 to Img.Width - 1 do begin
          C := Fast32Src[x, y];
          if C.C = clFuchsia then C.A := 0 else C.A := MaxByte;
          Fast32Src[x, y] := C;
        end;
      end;
    end;
  end;
  if Fast32Src.Attach(Img) and Fast32Dst.Attach(ColorBitmap) then begin
    for Y := 0 to Img.Height - 1 do begin
      for X := 0 to Img.Width - 1 do begin
        C := Fast32Src[x, y];
        Fast32Dst[x, y] := C;
...
begin
  ...
  if (Img.PixelFormat = pf32bit) and Fast32.Attach(Img) then begin
    BlackColor.I := 0;
    for x := 0 to Img.Width - 1 do
      for y := 0 to Img.Height - 1 do begin
        C := Fast32.Pixels[x, y];
        if (C.R = $FF) and (C.G = $FF) and (C.B = $FF) and (C.A = 0) then Fast32.Pixels[x, y] := BlackColor;
//        if C. {and $FFFFFFFF} = $FFFFFFFF then Fast32.Pixels[x, y] := BlackColor;
      end;
  end;
  FreeAndNil(Fast32);
end;
...
//      Stretch(SrcBmp, Image, Image.Width, Image.Height, ftMitchell);
