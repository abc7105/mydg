...
uses acntUtils, CommCtrl, ShellAPI, sConst, acPNG, sVCLUtils, sGraphUtils {$IFNDEF ALITE}, sDialogs{$ELSE}, FileCtrl{$ENDIF};
...
var
  dx : integer;
begin
  if X > FormImgListEditor.ListView1.Width - scBorder
    then dx := 10
    else if X < scBorder
      then dx := -10
      else dx := 0;
...
begin
  ...
  if (X < scBorder) or (X > ListView1.Width - scBorder) then StartScroll
end;
...
begin
  ...
  Height := 94 + ListView1.Height;
  Constraints.MinHeight := Height;
  sBitBtn7.Top := ListView1.BoundsRect.Bottom - sBitBtn7.Height - 4;
...
begin
  inherited;
  if Key = 46 then SpeedButton2.Click;
end;
...
{$ENDIF}
    for j := 0 to ListView1.Items.Count - 1 do if (ImageList1.Items[j].ImgData <> nil) then begin
      if ImageList1.Items[j].ImageFormat = ifPNG
        then ImageList1.Items[j].ImgData.SaveToFile(sPath + '\' + IntToStr(j) + '.' + acPngExt)
        else ImageList1.Items[j].ImgData.SaveToFile(sPath + '\' + IntToStr(j) + '.' + acIcoExt)
    end;
  end;
{$IFNDEF ALITE}
  PathDlg.Free;
...
begin
  if (ListView1.Selected <> nil) then begin
    ...
    if SaveDialog1.Execute then ImageList1.Items[j].ImgData.SaveToFile(SaveDialog1.FileName);
  end;
end;
