...
uses
  Windows, Messages, Graphics, Forms, Menus, Classes, Controls, acThumbForm,
...
{$ENDIF}
  ...
  published
    ...
    property Width : TMagnSize read FWidth write FWidth default 280;
    property Height : TMagnSize read FHeight write FHeight default 280;
...
//    SetWindowLong(TacMagnForm(acMagnForm).Handle, GWL_EXSTYLE, GetWindowLong(acMagnForm.Handle, GWL_EXSTYLE) or WS_EX_TOPMOST and not WS_EX_CONTROLPARENT);
  end;
end;
...
{$ENDIF}
  ...
  case Message.Msg of
    WM_WINDOWPOSCHANGED : begin
      SetWindowPos(Handle, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOACTIVATE or SWP_NOSIZE or SWP_NOMOVE or SWP_NOOWNERZORDER);
    end;
...
begin
  ...
  if FTempBmp <> nil then begin
    ...
    if FastDst.Attach(AlphaBmp) then for y := mRect.Top to mRect.Bottom do begin // Form must be non-fully transparent for a mouse catching
      Dst := AlphaBmp.ScanLine[y];
      for x := mRect.Left to mRect.Right do begin
        CDst := Dst[x];
...
//    if not IntUpdating then UpdateAero;
  end
end;
