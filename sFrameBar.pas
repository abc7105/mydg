...
uses
  ...
  ExtCtrls, sSpeedButton, sScrollBox, ImgList, Menus, sConst;
...
{$ENDIF} // NOTFORHELP
    procedure ArrangeTitles; virtual;
...
uses sMessages, sSkinProps, sVCLUtils, sFrameAdapter, sLabel, stdctrls, acntUtils, acSBUtils;
...
begin
  if SkinData.Skinned then begin
    if Assigned(ListSW) and (ListSW.sBarVert <> nil) and ListSW.sBarVert.fScrollVisible then Result := ListSW.sBarVert.ScrollInfo.nPos else Result := 0;
  end
  else begin
    sbi.cbSize := SizeOf(TScrollInfo);
    sbi.fMask := SIF_POS;
    GetScrollInfo(Handle, SB_VERT, sbi);
    Result := sbi.nPos
  end;
end;
...
begin
  if FCursor <> Value then begin
    FCursor := Value;
    if TitleButton <> nil then TitleButton.Cursor := Value;
  end;
end;
...
begin
  ...
  if Active and SkinData.Skinned and SkinData.SkinManager.IsValidImgIndex(SkinData.BorderIndex) then begin
    case Result of
      0 : if SkinData.SkinManager.ma[SkinData.BorderIndex].ImageCount > 3 then Result := 3 else Result := 1
      else if SkinData.SkinManager.ma[SkinData.BorderIndex].ImageCount > 3 then Result := 3;
    end;
  end;
end;
