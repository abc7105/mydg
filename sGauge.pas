...
begin
  ...
  RtSrc := nil;
  if not IsVert or Animated or not FCommonData.SkinManager.IsValidSkinIndex(Index) then begin
    ...
    if FillSize > 0 then if FCommonData.SkinManager.IsValidSkinIndex(Index) then begin
      ...
      if Animated and (FAnimPos < Fillsize + AnimSize) then begin // Prepare mask of region
        ...
        if not SkinData.SkinManager.gd[SkinData.SkinIndex].ReservedBoolean then begin
          ...
          if (WidthOf(RR) > 0) and FCommonData.SkinManager.IsValidImgIndex(RgnIndex) then begin
            ...
            MaskBmp := CreateBmp32(WidthOf(RR, True), HeightOf(RR, True));
      ...
      if (FTmpProgressBmp <> nil) and (TempBmp.Width = FTmpProgressBmp.Width) and (TempBmp.Height = FTmpProgressBmp.Height) then begin
        BitBlt(TempBmp.Canvas.Handle, 0, 0, TempBmp.Width, TempBmp.Height, FTmpProgressBmp.Canvas.Handle, 0, 0, SRCCOPY);
      end
      else begin
        if SkinData.SkinManager.gd[SkinData.SkinIndex].ReservedBoolean
          then PaintItemBG(Index, pSkinSection, CI, 0, Rect(0, 0, FillSize, H), Point(0, 0), TempBmp, FCommonData.SkinManager)
          else PaintItem(Index, pSkinSection, CI, True, 0, Rect(0, 0, FillSize, H), Point(0, 0), TempBmp, FCommonData.SkinManager);
        if FTmpProgressBmp = nil then FTmpProgressBmp := CreateBmp32(TempBmp.Width, TempBmp.Height) else begin
      ...
      if Animated and (FAnimPos < Fillsize + AnimSize) then begin
        if (WidthOf(RR) > 0) and FCommonData.SkinManager.IsValidImgIndex(RgnIndex) and not SkinData.SkinManager.gd[SkinData.SkinIndex].ReservedBoolean then begin
          SrcBmp := CreateBmp32(WidthOf(RR, True), HeightOf(RR, True));
          LightBmp := CreateBmp32(WidthOf(RR, True), HeightOf(RR, True));
          ...
          SumByMaskWith32(SrcBmp, LightBmp, MaskBmp, Rect(0, 0, WidthOf(RR, True), HeightOf(RR, True)));
...
begin
  ...
  if (FTmpProgressBmp <> nil) then FreeAndNil(FTmpProgressBmp);
  inherited Destroy;
end;
...
begin
  ...
  if SkinData.SkinManager.gd[SkinData.SkinIndex].ReservedBoolean and (Kind in [gkHorizontalBar, gkVerticalBar]) then begin
    PaintItemBG(FCommonData, CI, 0, Rect(0, 0, width, Height), Point(Left, Top), FCommonData.FCacheBMP);
    SkinPaintBody(Rect(0, 0, Width, Height));
    if SkinData.SkinManager.IsValidImgIndex(SkinData.SkinManager.gd[SkinData.SkinIndex].BorderIndex)
      then DrawSkinrect(FCommonData.FCacheBMP, Rect(0, 0, width, Height), True, CI, SkinData.SkinManager.ma[SkinData.SkinManager.gd[SkinData.SkinIndex].BorderIndex], 0, False, SkinData.SkinManager);
  end
  else begin
  ...
  with AnImage.Canvas do begin
    ...
    case FKind of
      gkHorizontalBar: begin
        ...
        if FillSize > 0 then FillRect(Rect(PaintRect.Left, PaintRect.Top, integer(FillSize), H));
      end;
      gkVerticalBar: begin
        FillSize := SolveForX(PercentDone, H);
