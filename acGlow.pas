...
begin
  if Wnd <> nil then begin
    ...
    if csDestroying in SkinManager.ComponentState then FreeAndNil(Wnd) else Wnd.Release;
    Wnd := nil;
  end;
  inherited;
end;
...
begin
  if Wnd = nil then begin
    ...
    SetWindowLong(Wnd.Handle, GWL_EXSTYLE, GetWindowLong(Wnd.Handle, GWL_EXSTYLE) or WS_EX_LAYERED or WS_EX_NOACTIVATE or WS_EX_TRANSPARENT or WS_EX_TOOLWINDOW);
    SetWindowPos(Wnd.Handle, h, R.Left - SkinManager.MaskWidthLeft(MaskIndex) + Margin + dx1, R.Top - SkinManager.MaskWidthTop(MaskIndex) + Margin + dy1,
