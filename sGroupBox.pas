...
begin
  ...
  acWriteTextEx(FCommonData.FCacheBMP.Canvas, PacChar(Caption), True, R, Flags, SkinData, False);
end;
...
var
  ...
  ALeft: Integer;
begin
  if (FButtons.Count <> 0) and not FReading and not (csLoading in ComponentState) then
  begin
    DC := GetDC(0);
...
//    TopMargin := HeightOf(CaptionRect) + 2; !!!
    ...
    try
      ...
      with TsGroupButton(FButtons[I]) do
        begin
