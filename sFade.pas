...
uses sGraphUtils, Forms, Controls, sMaskData, sMessages, math, acntUtils, sStyleSimply, acGlow, sSkinProps, sVCLUtils;
...
begin
  if CommonData.SkinManager.gd[CommonData.SkinIndex].FadingEnabled then begin
    ...
    if Timer <> nil then Timer.Direction := Direction;
  end;
end;
...
begin
  if (FOwnerData.FOwnerControl = nil) or (csDestroying in FOwnerData.FOwnerControl.ComponentState) or (Iterations = 0) or FadingForbidden then Exit;
  if not FOwnerData.FOwnerControl.Enabled then begin Enabled := False; Iterations := 0; FOwnerData.Invalidate; Exit end;
  b := TBitmap.Create;
  c.R := IntToByte(MaxByte - (FadeLevel shl 8) div Iterations); c.G := c.R; c.B := c.R; c.A := 0;
  b.Assign(BmpFrom);
...
begin
  FOwnerData := Value;
  if FOwnerData.BGChanged and FOwnerData.FCacheBmp.Empty and (FOwnerData.FOwnerControl <> nil) then SendAMessage(TWinControl(FOwnerData.FOwnerControl).Handle, AC_PREPARECACHE);
...
begin
  if Assigned(Self) and Assigned(FOwnerData) and not (csDestroying in ComponentState) and not ToEnd and not FadingForbidden then case FDirection of
    fdUp, fdDown : Change;
  end;
end;
...
begin
  try
    ...
    FadeInProcess := True;
    c.R := IntToByte(MaxByte - (FadeLevel shl 8) div Iterations); 
...
begin
  ...
  try // Checking if must be finished
