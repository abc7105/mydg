...
interface
uses
  Classes, ADODB, dialogs, CLSLXY, Variants, excel2000,
...
type
  ...
  private
    procedure importPZB();
    procedure editFILEDSTR(ATABLE: TADOTable; FIELDNAME: string; VALUE: Variant);
    procedure editFILEDnumber(ATABLE: TADOTable; FIELDNAME: string; VALUE: Variant);
  public
    procedure import();
  end;
...
begin
  ...
  if (MonthOf(axm.startrq) = 1) and (MonthOf(axm.endrq) = 12)
    ...
    axm.yeard := IntToStr(YEAROf(axm.startrq)) + '年' + IntToStr(monthOf(axm.startrq)) + '月' + '至' +
...
begin
  ...
  begin
    ...
    aworkbook := fxlsapp.workbooks.open(fxm.mbpath + '\' + dgname, EmptyParam, EmptyParam, EmptyParam, EmptyParam,
    ...
    begin
      ...
      if (Pos('检查情况表', fxlsapp.activeworkbook.sheets.item[j].name) > 0) then
      begin
        fxlsapp.activeworkbook.sheets.item[j].activate;
        sheettmp := fxlsapp.activeworkbook.activesheet;
    ...
    fxlsapp.ActiveWorkbook.SaveAs(fxm.xmpath + '\' + dgname, EmptyParam, EmptyParam, EmptyParam,
...
begin
  ...
  qrytmp.SQL.Add('  and xmid=''' + trim(axm.xmid) + '''  group by 底稿名称 order by 底稿名称');
  qrytmp.Open;
...
begin
  ...
  qrydg.SQL.Add('  and xmid=''' + trim(fxm.xmid) + '''  group by 底稿名称 order by 底稿名称');
  qrydg.Open;
...
begin
  ...
  qrytmp.SQL.Add(' select 代码 from dg7 where len(trim(代码)) =' +
    ...
    'and  核算项目名称 is null and 期初=0 and 期末=0 and 借方发生=0 and 贷方发生=0 and xmid=''' + trim(axm.xmid) +
    '''');
  qrytmp.SQL.Add(')');
  qrytmp.ExecSQL;
...
begin
  ...
  // tabletmp := TADOTable.create(nil);
  // tabletmp.connection := qrytmp.connection;
...
begin
  ...
  begin
    ...
    lxyexcel.fillacell(sheettmp, bColumnSDB.kmdm, currentline, qrytmp.fieldbyname('代码').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.kmname, currentline, qrytmp.fieldbyname('长科目名').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.KMDIRECT, currentline, qrytmp.fieldbyname('借贷方向').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.qc, currentline, qrytmp.fieldbyname('期初').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.SDqc, currentline, qrytmp.fieldbyname('期初').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.qM, currentline, qrytmp.fieldbyname('期末').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.SDqM, currentline, qrytmp.fieldbyname('期末').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.jffs, currentline, qrytmp.fieldbyname('借方发生').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.dffs, currentline, qrytmp.fieldbyname('贷方发生').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.sdFS, currentline, qrytmp.fieldbyname('贷方发生').asstring);
...
begin
  ...
  begin
    ...
    begin
      ...
      lxyexcel.fillacell(sheettmp, bColumnSDB.kmdirect, currentline, km[i, 3]);
      ...
      begin
        cellformula := lxyexcel.rowSUM(bColumnSDB.qm, bColumnSDB.qmtzjf, bColumnSDB.qmtzdf, bColumnSDB.qmcfljf,
          bColumnSDB.qmcfldf,
      ...
      begin
        cellformula := lxyexcel.rowSUM(bColumnSDB.fs, bColumnSDB.qmtzjf, bColumnSDB.qmtzdf, bColumnSDB.qmcfljf,
          bColumnSDB.qmcfldf,
          currentline, km[i, 3]);
...
procedure import_PZB.editFILEDSTR(ATABLE: TADOTable; FIELDNAME: string; VALUE: Variant);
begin
  //
  try
...
var
  ...
  icount: integer;
begin
  //  if not sheetexists(FXLSAPP,PZsheet) then
  //  begin
...
begin
  ...
  begin
    ...
    lxyexcel.fillacell(sheettmp, bColumnSDB.pzdate, currentline, qrytmp.fieldbyname('日期').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.pztype, currentline, qrytmp.fieldbyname('凭证类型').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.pznumber, currentline, qrytmp.fieldbyname('凭证编号').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.pzmemo, currentline, qrytmp.fieldbyname('摘要').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.pzkm, currentline, qrytmp.fieldbyname('科目名称').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.pzmirrorkm, currentline, qrytmp.fieldbyname('对方科目').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.pzjf, currentline, qrytmp.fieldbyname('借方').asstring);
    lxyexcel.fillacell(sheettmp, bColumnSDB.pzdf, currentline, qrytmp.fieldbyname('贷方').asstring);
    currentline := currentline + 1;
...
begin
  ...
  DOSQL('UPDATE DG7 SET  一级科目代码=left(代码,' + Trim(IntToStr(axm.kmlen)) + ') WHERE  XMID=''' + XMID + '''');
