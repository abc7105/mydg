...
uses
  ...
  StdCtrls, Mask, acntUtils, buttons, menus, {$IFNDEF ALITE} sCalcUnit, {$ENDIF}
  sConst, sSpeedButton, sGraphUtils, sCommonData, sDefaults, sMaskEdit, sSkinProps, sBitBtn,
...
uses sStyleSimply, {$IFNDEF ALITE} sCurrencyEdit, {$ENDIF} {$IFDEF CHECKXP} UxTheme, Themes, {$ENDIF}
  sMaskData, sVCLUtils, sMessages, sAlphaGraph, sSkinManager, math {$IFDEF LOGGED}, sDebugMsgs{$ENDIF};
...
{$ENDIF}
  if Message.MSG = SM_ALPHACMD then case Message.WParamHi of
    ...
    WM_PRINT : if SkinData.Skinned then begin
      ...
      if ShowButton then FButton.PaintTo(DC, Point(FButton.Left + 2, FButton.Top + 2));
      FButton.Visible := b;
      Exit;
    end;
  end;
  ...
  if Message.MSG = SM_ALPHACMD then case Message.WParamHi of
    ...
    CM_ENABLEDCHANGED : if Assigned(Button) then Button.Enabled := Enabled;
...
begin
  ...
  if Assigned(FButton) then begin
    if not RestrictDrawing then FButton.SkinData.BGChanged := True;
  ...
  try
    if SkinData.Skinned then begin
      ...
      if not SkinData.Updating then begin
        ...
        if (not ControlIsActive(SkinData) or (SkinData.PrintDC = NewDC)) and FButton.Visible
...
begin
  ...
  else if not (InAnimationProcess and (Canvas.Handle <> SkinData.PrintDC)) then inherited; // Animation under Vista fix
end;
