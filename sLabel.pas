  ...
  if not (csDestroying in ComponentState) then begin
    if not FontChanging then begin
      ...
      if (SkinSection <> '') and (DefaultManager <> nil) and DefaultManager.Active then begin
        ...
        if (si >= 0) and (Font.Color <> DefaultManager.gd[si].FontColor[1]) then Font.Color := DefaultManager.gd[si].FontColor[1];
      end;
      FontChanging := False;
    end;
    if (SkinSection <> '') and (DefaultManager <> nil) and DefaultManager.Active then begin
...
begin
  ...
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    AC_REMOVESKIN : if not (csDestroying in ComponentState) and (DefaultManager <> nil) and (Message.LParam = LongInt(DefaultManager)) and (Font.Color <> clWindowText) then Font.Color := clWindowText;
...
begin
  ...
  if MouseAbove and not (csDesigning in ComponentState) then begin
    ...
    Result := FNormalFont;
    if UseSkinColor and (DefManager <> nil) and DefManager.Active and DefManager.IsValidSkinIndex(DefManager.ConstData.IndexGLobalInfo) then C := DefManager.gd[DefManager.ConstData.IndexGLobalInfo].FontColor[5] else C := FNormalFont.Color; // TCBottom
    if C <> -1 then Result.Color := C else Result.Color := clBlue;
  end;
end;
...
begin
  if FShadow.Mode <> smNone then begin // If not standard kind
    ...
    case FShadow.Mode of
      ...
      smSkin1 : begin
        if (DefaultManager <> nil) and DefaultManager.SkinData.Active and (DefaultManager.SkinData.Shadow1Blur <> -1) then begin
...
begin
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    AC_REMOVESKIN : begin
      if not (csDestroying in ComponentState) and (DefaultManager <> nil) and (Message.LParam = LongInt(DefaultManager)) and UseSkinColor {$IFNDEF SKININDESIGN}and not (csDestroying in ComponentState) and not ((csDesigning in ComponentState) and (GetOwnerFrame(Self) <> nil)){$ENDIF} and (Font.Color <> clWindowText)
