...
uses sGraphUtils, sVCLUtils, sMessages, acntUtils, sMAskData, sAlphaGraph, sStyleSimply, sSkinProps, acGlow,
...
begin
  with Button do begin
    R := BoundsRect;
    for i := 0 to Parent.ControlCount - 1 do if (Parent.Controls[i] <> Button) and Parent.Controls[i].Visible then begin
...
begin
  if not GlowingAllowed(Button) then Exit;
...
begin
  ...
  case Layout of
    ...
    blGlyphRight : case Alignment of
      taLeftJustify : begin
        x := Width - ArrowWidth - GenMargin - 2 * dw - Spacing * integer(ShowCaption and (gw > 0) and (Caption <> '')) - gw;
        y := (Height - gh) div 2;
      end;
      taCenter : begin
    ...
    tbsDivider: begin
      ...
      if FCommonData.SkinManager.IsValidSkinIndex(si) then begin
        ...
        if Assigned(FCommonData.SkinManager) and FCommonData.SkinManager.IsValidImgIndex(mi) then begin
          w := WidthOfImage(SkinData.SkinManager.ma[mi]);
    ...
    tbsSeparator: begin
      ...
      if FButtonStyle = tbsDropDown then begin
        ...
        if FCommonData.SkinManager.ConstData.MaskArrowBottom > -1 then with FCommonData.SkinManager do begin
          ...
          x := R.Left + (ArrowWidth - WidthOf(FCommonData.SkinManager.ma[ConstData.MaskArrowBottom].R) div FCommonData.SkinManager.ma[ConstData.MaskArrowBottom].ImageCount) div 2 + 1;
