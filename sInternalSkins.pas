...
uses
  ...
  ExtCtrls, sPanel, sButton, sListBox;
...
uses IniFiles, sStrings, sDefaults, sConst{$IFNDEF ALITE}, acSelectSkin{$ENDIF};
...
begin
  ...
  if FileExists(s) then begin
    IntName := NewName + ' ' + acs_InternalSkin;

    for i := 0 to SkinManager.InternalSkins.Count - 1 do if SkinManager.InternalSkins[i].Name = IntName then Exit;
    ...
    s := s + '.' + acSkinExt + ' file with a skin data does not exists';
    ShowWarning(s);
  end;
end;
...
{$IFNDEF ALITE}
  ...
  if NewName <> '' then sl.Add(NewName);
...
{$ELSE}
  ...
  if fod.Execute then begin
    ...
    SkinManager.InternalSkins.Add;
    SkinManager.InternalSkins[SkinManager.InternalSkins.Count - 1].Name := NewName + ' ' + acs_InternalSkin;
...
begin
  ...
  if not acDirExists(NewDir) then NewDir := DefSkinsDir;
