...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, ExtCtrls, sMonthCalendar, comctrls, sConst, sPanel, acntUtils
  {$IFDEF DELPHI6UP}, Variants{$ENDIF}, sCustomComboEdit, sSkinProvider, Menus;
...
begin
  if (csDesigning in ComponentState) then Result := VarFromDateTime(SysUtils.Date) else Result := VarFromDateTime(FCalendar.CalendarDate);
end;
...
begin
  if not (csDesigning in ComponentState) then begin
    try
      if (Trim(ReplaceStr(VarToStr(Value), DateSeparator, '')) = '') or VarIsNull(Value) or VarIsEmpty(Value)
        then FCalendar.CalendarDate := VarToDateTime(SysUtils.Date)
  ...
  case Key of
    VK_RETURN: begin
      if FEditor <> nil then begin
        ...
        if not IsValidDate(d) then Exit;
        CanAccept := True;
...
begin
  Result := True;
  if (TsCustomDateEdit(FEditor).MinDate <> 0) and (Date < TsCustomDateEdit(FEditor).MinDate) then Result := False;
  if (TsCustomDateEdit(FEditor).MaxDate <> 0) and (Date > TsCustomDateEdit(FEditor).MaxDate) then Result := False;
end;
