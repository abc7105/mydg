...
begin
  if Caption <> '' then begin
    ...
    if WordWrap then Fmt := Fmt or DT_WORDBREAK or DT_CENTER else Fmt := Fmt or DT_SINGLELINE;
...
{ v6.33
  if (Parent is TsRadioGroup) then begin
    Result := FReadOnly not TsRadioGroup(Parent).CanModify(TsRadioGroup(Parent).FButtons.IndexOf(Self))
  end
  else
}
  Result := FReadOnly;
...
begin
  ...
  if Checked then FCommonData.Invalidate;
end;
...
begin
  ...
  if AutoSize then AdjustSize;
  if not Checked then FCommonData.Invalidate;
end;
...
{$ENDIF}
        ...
        WM_LBUTTONUP : if not (csDesigning in ComponentState) and Enabled then begin
          if not ReadOnly and CanModify and FPressed then Checked := True;
          FPressed := False;
          Repaint;
