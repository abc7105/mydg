...
uses sStyleSimply, sPageControl, sMessages, sGraphUtils, sSkinProps, sMaskData, acntUtils, sAlphaGraph,
  sVclUtils, sSkinManager, sCommonData{$IFDEF LOGGED}, sDebugMsgs{$ENDIF};
...
{$ENDIF}
  ...
  Case Message.Msg of
    WM_LBUTTONDBLCLK, WM_NCLBUTTONDBLCLK : begin
      inherited;
      Pressed := True;
      if (DrawingState = dsPrevUp) and (Position < Max)
        then DrawingState := dsPrevDown
        else if (DrawingState = dsNextUp) and (Position > Min) then DrawingState := dsNextDown;
    end;
    WM_NCHITTEST : begin
      inherited;
    ...
    WM_PRINT : SendMessage(Handle, WM_PAINT, Message.WParam, Message.LParam);
    WM_PAINT : begin
