...
{$IFDEF ALITE}
        if sListBox1.ItemIndex <> 0
          then sListBox2.ItemEnabled[sListBox2.Items.Count - 1] := False
            ...
            if (s2 <> acThirdCaptions[ord(tpEdit)]) and
               (s2 <> acThirdCaptions[ord(tpButton)]) and
               (s2 <> acThirdCaptions[ord(tpGrid)]) and
               (s2 <> acThirdCaptions[ord(tpListView)]) and
               (s2 <> acThirdCaptions[ord(tpTreeView)]) and
               (s2 <> acThirdCaptions[ord(tpStatusBar)])
              then sListBox2.ItemEnabled[sListBox2.Items.Count - 1] := False
              else
{$ENDIF}
