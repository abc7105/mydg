...
procedure acSetAnimation(Value: Boolean);
...
function AnsiProperCase(const S: AnsiString; const WordDelims: TSysCharSet): AnsiString;
...
procedure CopyFiles(SrcMask, SrcDir: ACString; DstDir: ACString);
...
uses
  ...
  {$IFNDEF ALITE}sDialogs, {$ENDIF} Dialogs, Math, ShellAPI, TypInfo, sSkinProvider;
...
function Between(Value, i1, i2 : integer) : boolean;
begin
  if i1 < i2 then Result := (Value >= i1) and (Value <= i2) else Result := (Value <= i1) and (Value >= i2);
end;
...
function SubStrInclude(const SubString, s : string; CaseInsensitive : boolean) : boolean;
begin
  if CaseInsensitive then Result := pos(UpperCase(SubString), UpperCase(s)) > 0 else Result := pos(SubString, s) > 0;
end;
...
function IsIDERunning: boolean;
begin
  if Assigned(acntUtils.IsDebuggerPresent) then Result := acntUtils.IsDebuggerPresent else Result := True;
end;
  ...
  try
    while DosCode = 0 do begin
      if (FileInfo.Name[1] <> '.') and (FileInfo.Attr <> faVolumeID) then begin
        if (FileInfo.Attr and faVolumeID <> faVolumeID) then acCopyFile(SrcDir + FileInfo.Name, DstDir + FileInfo.Name, False);
      end;
...
var i: Integer;
begin
  if nCount < 1 then S := '' else begin
    if Length(S) <> nCount then SetLength(S, nCount);
...
initialization
  if hKern32 = 0 then hKern32 := LoadLibrary(kernel32);
  if hKern32 <> 0 then begin
    if not Assigned(IsDebuggerPresent) then IsDebuggerPresent := GetProcAddress(hKern32, 'IsDebuggerPresent');
    IsWow64Process := GetProcAddress(hKern32, 'IsWow64Process');
    if IsWOW64Proc and not AeroIsEnabled then x64woAero := True;
  end;

finalization
  if hKern32 <> 0 then FreeLibrary(hKern32);
