...
begin
  ...
  if UseRightToLeftReading then Flags := Flags or DT_RIGHT;
...
procedure TsStatusBar.InternalDrawPanel(Panel: {$IFDEF TNTUNICODE}TTntStatusPanel{$ELSE}TStatusPanel{$ENDIF}; Text: acString; Rect: TRect);
var
  SavedDC : hdc;
  aRect: TRect;
...
begin
  ...
  if Assigned(Panel) and (Panel.Style = psOwnerDraw) and Assigned(OnDrawPanel) then begin
    ...
    if Assigned(Panel)
      then DoDrawText(Panel.Text, aRect, GetStringFlags(Self, Panel.Alignment) or DT_END_ELLIPSIS)
      else DoDrawText(Text, aRect, GetStringFlags(Self, taLeftJustify) or DT_END_ELLIPSIS);
  end;
end;
...
var
  ...
  begin
    Result := point(Width - WidthOfImage(FCommonData.SkinManager.ma[i]) - BorderWidth, Height - HeightOfImage(FCommonData.SkinManager.ma[i]) - BorderWidth)
  end;
begin
...
var
  i, MaxWidth: integer;
begin
  if SizeGrip then MaxWidth := Width - 18 else MaxWidth := Width;
  if SimplePanel or (Panels.Count = 0)
    then InternalDrawPanel(nil, SimpleText, Rect(0, 1, MaxWidth - 1, Height - 1))
    else for i := 0 to Panels.Count - 1 do begin
      DrawPanel(Panels[i], Rect(PanelOffset(i), 0, iffi(i <> Panels.Count - 1, PanelOffset(i) + Panels[i].Width, MaxWidth), Height));
    end
end;
