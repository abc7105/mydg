...
{$IFNDEF NOTFORHELP}
    property OnDrawDay : TGetCellParams read FOnDrawDay write FOnDrawDay; // Must be hidden later
{$ENDIF}
...
//  DontFoldersGenerate := True;
  ...
  else PathDialogForm.sLabel1.Visible := False;
  PathDialogForm.UpdateAnchors;

//  DontFoldersGenerate := False;
  try
...
var
  s: string;
begin
  if FileExists(AFileName) then s := ExtractFilePath(AFileName) else s := AFileName;
  if (Text = '') or not MultipleDirs then Text := s else Text := Text + ';' + s;
end;
...
var
  ...
  Pos: Integer;
begin
  if not MultipleDirs then Result := LongToShortPath(Text) else begin
    Result := '';
    Pos := 1;
    while Pos <= Length(Text) do begin
...
begin
  if not (csDesigning in ComponentState) and CheckOnExit and CheckValidDate then begin
    if (FMaxDate <> 0) and (Date > FMaxDate) then Date := FMaxDate else if (FMinDate <> 0) and (Date < FMinDate) then Date := FMinDate;
  end;
  inherited;
end;
...
begin
  TStringList(FCalendarHints).OnChange := nil;
  try
    while (FCalendarHints.Count > 4) do FCalendarHints.Delete(FCalendarHints.Count - 1);
  finally
    TStringList(FCalendarHints).OnChange := CalendarHintsChanged;
  end;
...
begin
  if CharInSet(Key, ['T', 't', '+', '-']) and DroppedDown then begin
...
begin
  if FShowCurrentDate <> Value then FShowCurrentDate := Value;
end;
...
procedure DateFormatChanged;
  ...
  var
    I: Integer;
  begin
    with AControl do for I := 0 to ControlCount - 1 do if Controls[I] is TsCustomDateEdit then TsCustomDateEdit(Controls[I]).UpdateMask else if Controls[I] is TWinControl then IterateControls(TWinControl(Controls[I]));
  end;
var
  I: Integer;
begin
